...
extension String {
    ...
    /// Checks if the CVV number is valid based on the expected length.
    /// - Parameter expectedLength: The expected length of the CVV, typically 3 or 4.
    /// - Returns: The CVV if valid, otherwise nil.
    func checkCVV(expectedLength: Int = 3) -> String? {
        let cleanedCVV = self.sanitizedNumericString
        guard cleanedCVV.count == expectedLength, cleanedCVV.isNumeric else {
            return nil
        }
        return cleanedCVV
    }
    
    /// Extracts and validates the CVV from a string.
    /// - Returns: The first valid CVV found, or nil if none are valid.
    func extractCVV() -> String? {
        print("Extracting CVV from: \(self)")
        
        // CVV extraction regex for common lengths
        let pattern = "(\\b\\d{3}\\b)|(\\b\\d{4}\\b)"
        ...
        for result in results {
            if let range = Range(result.range, in: self) {
                let extractedCVV = String(self[range])
                if extractedCVV.isNumeric, (extractedCVV.count == 3 || extractedCVV.count == 4) {
                    return extractedCVV
                }
            }
        }
